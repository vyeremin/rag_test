...
def generate_response_with_template(query: str, results: List[Tuple[int, float]], document_mapping: Dict[int, str]) -> str:
    """
    ...
    for idx, score in results:
        ...
        if top_score - score > SENTENCE_SCORE_DROP_THRESHOLD:
            logger.debug("Score drop detected at Document ID %s: top score %.4f, current score %.4f", idx, top_score, score)
            break
        response_parts.append(document_mapping[idx])

    if add_concern_statement:
        response_parts.append("If a caregiver has concerns, they should speak with a pediatric professional for personalized guidance.")
